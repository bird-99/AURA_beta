...
function testMinimalContainerFirst() {
  const css = buildComfortCss('MINIMAL', 1, '.aura-scope', { darkModeEnabled: true });
  ...
  assert(css.includes('.aura-scope { color-scheme: dark'), 'MINIMAL Comfort dark mode should activate when requested');
...
function testScopedSemanticMeasure() {
  const css = buildComfortCss('SCOPED', 1, '.aura-scope', { darkModeEnabled: true });
  ...
  assert(css.includes('.aura-scope { color-scheme: dark'), 'SCOPED Comfort dark mode should activate when requested');
...
function testFocusScopedResponsive() {
  ...
  assert(!css.includes('.aura-focus-hide'), 'FOCUS scoped should not hide content by default');
  assert(!/display:\s*none/i.test(css), 'FOCUS scoped should not emit display:none by default');
...
function testFocusMinimalResponsive() {
  ...
  assert(!css.includes('.aura-focus-hide'), 'FOCUS minimal should not hide content by default');
  assert(!/display:\s*none/i.test(css), 'FOCUS minimal should not emit display:none by default');
