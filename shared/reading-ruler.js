...
export function getExistingReadingRulerRoot(doc) {
...
export function createReadingRulerRoot(doc, options = {}) {
...
export function setReadingRulerVisible(handle, visible) {
  if (!handle?.root) return;
  handle.root.style.display = visible ? 'block' : 'none';
}

export function updateReadingRuler(handle, options = {}) {
...
export function destroyReadingRuler(handle) {
  if (!handle?.root) return;
  handle.root.remove();
}

export function createReadingRulerController(doc, options = {}) {
  const view = doc?.defaultView || globalThis;
  let handle = null;
  let enabled = false;
...
export const READING_RULER_DEFAULTS = Object.freeze({
