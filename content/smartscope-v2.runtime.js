 ...
 */
function computeMetricsCheap(input) {
 ...
 */
function computeMetricsExpensive(input) {
  const { el, deadline, now } = input || {};
  const clock = typeof now === 'function' ? now : defaultNow;
 ...
 * @param {Element} el
 */
function verifyFinalScopeEl(el) {
 ...
 * @returns {boolean}
 */
function isExcludedCandidate(el) {
 ...
 */
async function detectScopeRootV2(input) {
...
function detectSmartScopeV2(doc, options = {}) {
...
const smartScopeV2Constants = {
  ...
  globalThis.AURA_SMARTSCOPE_V2 = Object.freeze({
    DEFAULT_BUDGET_MS,
    MAX_CANDIDATES,
    MAX_NODES_SCANNED,
    TOP_K,
